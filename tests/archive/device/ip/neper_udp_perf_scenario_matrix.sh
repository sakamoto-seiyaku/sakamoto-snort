...
RUNNER="$IPMOD_DIR/neper_udp_perf_3way.sh"
...
Options:
  ...
  --delay-ns <n>                 udp_stream delay in ns (default: ${DELAY_NS})
...
for v in ROUNDS_PER_SCENARIO DURATION_S WARMUP_S COOLDOWN_JOB_S SEED THREADS FLOWS BYTES DELAY_NS CLIENT_UID LOCAL_HOST_COUNT LOCAL_HOST_BASE_OCTET PERFMETRICS; do
  if ! [[ "${!v}" =~ ^[0-9]+$ ]]; then
...
if [[ -z "$OUTPUT_DIR" ]]; then
  OUTPUT_DIR="$IPMOD_DIR/records/neper-udp-perf-scenario-matrix-$(timestamp_compact_utc)_${serial}"
fi
