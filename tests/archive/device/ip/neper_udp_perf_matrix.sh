...
for v in ROUNDS DURATION_S WARMUP_S COOLDOWN_S COOLDOWN_ROUND_S THREADS FLOWS BYTES DELAY_NS CLIENT_UID LOCAL_HOST_COUNT LOCAL_HOST_BASE_OCTET PERFMETRICS; do
  if ! [[ "${!v}" =~ ^[0-9]+$ ]]; then
...
if [[ -z "$OUTPUT_DIR" ]]; then
  OUTPUT_DIR="$IPMOD_DIR/records/neper-udp-perf-matrix-$(timestamp_compact_utc)_${serial}"
fi
