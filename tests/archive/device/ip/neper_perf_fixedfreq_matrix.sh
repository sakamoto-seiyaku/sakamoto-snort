...
usage() {
  cat <<EOF
Usage: $0 [options] -- [neper_perf_matrix options]

This wrapper pins CPU frequencies (best-effort) and then runs neper_perf_matrix.
...
Example:
  bash $0 --serial 28201JEGR0XPAJ -- \\
    --rounds 5 --seconds 30 --warmup 10 --cooldown 30 \\
    --threads 8 --flows 1024 --bytes 1 --num-ports 16 \\
    --local-host-count 8 --perfmetrics 0
EOF
}
...
case "$MATRIX_MODE" in
  3way) MATRIX_RUNNER="$IPMOD_DIR/neper_perf_matrix.sh" ;;
  scenario) MATRIX_RUNNER="$IPMOD_DIR/neper_perf_scenario_matrix.sh" ;;
  *)
...
if [[ -z "$OUTPUT_DIR" ]]; then
  OUTPUT_DIR="$IPMOD_DIR/records/neper-perf-fixedfreq-matrix-$(timestamp_compact_utc)_${ADB_SERIAL_RESOLVED}"
fi
mkdir -p "$OUTPUT_DIR"
