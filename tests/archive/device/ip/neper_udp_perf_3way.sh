...
Options:
  ...
  --bytes <n>                udp_stream payload bytes (default: ${BYTES})
  --delay-ns <n>             udp_stream per-send delay (ns) (default: ${DELAY_NS})
...
for v in DURATION_S WARMUP_S COOLDOWN_S THREADS FLOWS BYTES DELAY_NS PORT CONTROL_PORT CLIENT_UID LOCAL_HOST_COUNT LOCAL_HOST_BASE_OCTET RULES_2K RULES_4K PERFMETRICS; do
  if ! [[ "${!v}" =~ ^[0-9]+$ ]]; then
...
records_dir="$IPMOD_DIR/records/neper-udp-perf-3way-${ts}_$(adb_target_desc)"
mkdir -p "$records_dir"
...
run_neper_udp_stream() {
  ...
  adb_su "ip netns exec \"$IPTEST_NS\" \"$IPTEST_NEPER_UDP_STREAM_DEVICE_BIN\" --nolog -4 -P \"$PORT\" -C \"$CONTROL_PORT\" -T \"$THREADS\" -F \"$FLOWS\" -B \"$BYTES\" >/data/local/tmp/iptest_neper_udp_srv.log 2>&1 &"
  sleep 0.2

  local client_cmd="\"$IPTEST_NEPER_UDP_STREAM_DEVICE_BIN\" --nolog -4 -c -H \"$IPTEST_PEER_IP\" -P \"$PORT\" -C \"$CONTROL_PORT\" -T \"$THREADS\" -F \"$FLOWS\" -B \"$BYTES\" -D \"$DELAY_NS\" -l \"$seconds\""
  if [[ -n "$local_hosts_csv" ]]; then
    client_cmd="${client_cmd} -L \"${local_hosts_csv}\""
  fi

  iptest_adb_as_uid "$CLIENT_UID" "$client_cmd" >"$client_log"
...
bytes_payload = int(sys.argv[7])
delay_ns = int(sys.argv[8])
uid = int(sys.argv[9])
...
def find_str(name: str) -> str:
    m = re.search(rf"^{re.escape(name)}=(.*)$", client_log, re.M)
    if not m:
        return ""
...
row = {
    ...
    "neper": {
        "throughput": (
            find_float("throughput")
            or find_float("remote_throughput")
            or find_float("local_throughput")
        ),
        "throughput_opt": find_str("throughput_opt"),
        "remote_throughput": find_float("remote_throughput"),
...
install_heavy_rules() {
  ...
  python3 - "$CLIENT_UID" "$subtable_start" "$subtable_count" "$IPTEST_HOST_IP" <<'PY' \
    | snortctl_batch \
...
for sp, dp in subtables:
    ...
    for _ in range(per_subtable - 1):
        print(
            ...
            f"dir=any proto=udp src={src} dst={dst} dport={miss_dport}"
        )
    print(
        f"IPRULES.ADD {uid} action=block log=1 enforce=0 priority={hit_prio} "
        f"dir=any proto=udp src={src} dst={dst} dport={hit_dport}"
    )
PY
import sys
...
if [[ "$WARMUP_S" -gt 0 ]]; then
  ...
  run_neper_udp_stream "warmup_off" "$WARMUP_S"
...
if [[ "$SCENARIO_MODE" == "all" || "$SCENARIO_MODE" == "off" ]]; then
  ...
  run_neper_udp_stream "iprules_off" "$DURATION_S"
...
if [[ "$SCENARIO_MODE" == "all" || "$SCENARIO_MODE" == "2k" ]]; then
  ...
  run_neper_udp_stream "iprules_2k" "$DURATION_S"
  if [[ "$COOLDOWN_S" -gt 0 && "$SCENARIO_MODE" == "all" ]]; then
    log "cooldown: ${COOLDOWN_S}s"
    sleep "$COOLDOWN_S"
  fi
...
if [[ "$SCENARIO_MODE" == "all" ]]; then
  ...
  run_neper_udp_stream "iprules_4k" "$DURATION_S"
