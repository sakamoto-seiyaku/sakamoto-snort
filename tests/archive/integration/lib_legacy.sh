#!/bin/bash
# tests/archive/integration/lib_legacy.sh - legacy 文本协议测试工具库（仅归档回查）
#
# NOTE:
# - 该文件保留历史 `60606` 文本协议（send_cmd / stream_sample / legacy asserts）。
# - 非归档主线入口请使用 `vNext`（`60607 + sucre-snort-ctl`）。
