...
Notes:
  - This sweep uses Tier-1 (netns+veth) via \`tests/device-modules/ip/cases/60_perf.sh\`.
  - Results are appended to a new records dir under \`tests/device-modules/ip/records/\`.
EOF
}
