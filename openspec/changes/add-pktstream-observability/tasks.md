...
- [ ] 3.6 开发冒烟：扩展 `tests/integration/full-smoke.sh`，验证 `HELP` 暴露 `METRICS.REASONS*`、`METRICS.REASONS` 返回有效 JSON、`METRICS.REASONS.RESET` 可清零，且 `RESETALL` 后 reason counters 归零
- [ ] 3.7 开发冒烟：扩展 `tests/integration/full-smoke.sh` 的 `PKTSTREAM` 采样，验证事件包含 `ipVersion/srcIp/dstIp/reasonId`，且不再包含 legacy `ipv4|ipv6`
