...
## Why
P0/P1 设计初衷需要对用户提供“友好可观测性”与“策略调试（log-only / would-match）”：
...
- 升级 PKTSTREAM Packet 事件 schema：
  ...
  - 移除：旧动态字段 `ipv4|ipv6`
  - 说明：除上述新增/移除字段外，其它既有字段保持不变（例如 `app/uid/userId/direction/interface/protocol/host/srcPort/dstPort/accepted`）。
...
## TBD (Next)
- 其它常态 counters/metrics（例如域名 `policySource` 计数、IP 规则 per-rule stats 的统一输出口径与复用）：TBD。域名侧的归因与 counters 口径已在 `docs/DOMAIN_POLICY_OBSERVABILITY.md` 固化（暂不新建域名相关 change）。

## Impact
- Affected docs（实现时）：`docs/INTERFACE_SPECIFICATION.md`
- Affected code（实现时）：`src/PacketListener.cpp`, `src/PacketManager.hpp`, `src/Packet.hpp`, `src/Packet.cpp`
