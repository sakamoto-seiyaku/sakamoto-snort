# perfmetrics-observability Specification

## Purpose
Define D-layer performance health metrics for sucre-snort:
- Runtime-toggleable sampling (`PERFMETRICS`, default disabled) with minimal overhead when disabled.
- Fixed-shape, pull-based perf snapshot (`METRICS.PERF`) in microseconds for regression/baseline comparisons.
- Stable metric boundaries for NFQUEUE packet service time (`nfq_total_us`) and DNS decision service time (`dns_decision_us`), plus stable reset semantics.
## Requirements
