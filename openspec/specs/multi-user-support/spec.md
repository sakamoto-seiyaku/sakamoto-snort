...
## Purpose
Define multi-user behavior for sucre-snort:
- App identity is the full Linux UID (includes Android `userId`).
...
### Requirement: vNext stream events expose uid and userId
DNS、数据包以及 Activity 流事件的 JSON 表示 MUST 包含 `uid` 与 `userId` 字段，使上层调用者可以按用户维度过滤或聚合事件。legacy `DNSSTREAM/PKTSTREAM/ACTIVITYSTREAM` 已冻结为 no-op，不再作为该能力的验证入口。

#### Scenario: DNS stream includes uid and userId
- **WHEN** 客户端使用 vNext `STREAM.START(type=dns)` 订阅 DNS 事件
...
- **WHEN** 客户端使用 vNext `STREAM.START(type=pkt)` 订阅数据包事件
...
- **WHEN** 客户端使用 vNext `STREAM.START(type=activity)` 订阅前台 Activity 事件
...
- **THEN** 系统 SHALL 使用该 UID 调用 `appManager.make(uid)` 更新前台 Activity；若存在 vNext activity stream 订阅，则事件包含对应 `uid`/`userId` 信息

### Requirement: Device-wide views reject USER filters
仍然保持设备级视图语义的命令（例如 `ALL<v>` / `<TYPE><v>`、`DOMAINS<v>` 等）MUST 不接受 `USER <userId>` 子句；如客户端传入 `USER` 子句，系统可以忽略该子句或返回错误，但不得 silently 改变这些命令的设备级语义。legacy stream 命令已冻结为 no-op；vNext stream 使用结构化 args，不使用 legacy `USER` 子句。
...
- 对仍然保持设备级视图语义且不支持 `USER` 过滤的命令（例如 `ALL<v>` / `<TYPE><v>`、`DOMAINS<v>` 等），HELP 输出 MUST 明确这些命令不接受 `USER <userId>` 子句；HELP 还 MUST 标明 legacy stream 命令已冻结/no-op，并指向 vNext `STREAM.START(type=...)`。
...
### Requirement: Legacy stream restore does not affect vNext streams
历史 DNS / Packet / Activity 流的持久化格式 MUST 不影响当前 vNext stream 语义：
- legacy stream 已冻结为 no-op，旧版本保存的流事件 MAY 被忽略；
- vNext stream 的 ring / pending state 仅为进程内观测会话态，不从 legacy stream save 文件恢复。

#### Scenario: Legacy DNS stream save does not replay into vNext
- **WHEN** 系统启动时存在旧版本遗留的 DNS 流持久化文件
- **THEN** 后续 vNext `STREAM.START(type=dns)` SHALL NOT replay those legacy persisted events
