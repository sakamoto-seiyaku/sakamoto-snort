# blockmask-chains Specification

## Purpose
Define combinable `blockMask` semantics for blocking lists and apps:
- Each blocking list uses a single-bit selector (`1/2/4/8/16/32/64`).
- Apps may combine multiple list bits in their app `blockMask`.
- Reinforced (`8`) implies standard (`1`) at the app-mask level.
- Domain list writes are listId-local (no cross-listId deduplication).
## Requirements
