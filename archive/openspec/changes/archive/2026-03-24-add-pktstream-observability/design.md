...
- `IP_LEAK_BLOCK`：legacy IP leak 拦截（`BLOCKIPLEAKS=1` 时生效；当前阶段默认关闭，暂不纳入基线验收）
- `ALLOW_DEFAULT`：默认放行
...
- 当前 P0 基线/验收以 `BLOCKIPLEAKS=0` 为前提（该附属功能在本阶段默认关闭，不作为阻塞项）。
- 为避免用户在开发态打开 `BLOCKIPLEAKS=1` 时出现“drop 但 reasonId 无法解释”的缺口，本阶段为其提供临时 reasonId：`IP_LEAK_BLOCK`（命名可在后续融合阶段调整）。

reasonId 选择必须确定且唯一；在当前 baseline（无规则引擎 override，且默认 `BLOCKIPLEAKS=0`）下遵循优先级：
`IFACE_BLOCK` > `ALLOW_DEFAULT`

当 `BLOCKIPLEAKS=1` 且触发 legacy IP leak 拦截时，优先级为：
...
3) **仅当 IP 规则层 `NoMatch`** 时，才允许回落到 legacy/domain 路径：在当前 A 层验收基线（可理解为 `BLOCKIPLEAKS=0`）下，该路径仅产生 `ALLOW_DEFAULT`；若开发态启用 `BLOCKIPLEAKS=1` 且触发 IP leak 拦截，则产生 `IP_LEAK_BLOCK`。  

> 注：`ip-leak` 与其它 legacy drop 原因的 reasonId 命名与优先级不作为本阶段验收主线，但为保证 `reasonId` 的“解释实际 verdict”契约不破洞，本 change 仍给出 `IP_LEAK_BLOCK` 作为临时 reasonId。若未来需要系统化收敛 legacy drop 原因的命名/优先级，必须以独立 change 增量定义。
