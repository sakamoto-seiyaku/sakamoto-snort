...
> 状态：已归档。当前 `Device / DX` 体系重组纲领见 `docs/testing/DEVICE_TEST_REORGANIZATION_CHARTER.md`；当前 IP 模组 active 入口与记录口径见 `docs/testing/ip/IP_TEST_MODULE.md`。
...
- **dx-smoke-datapath（快）**：< 2 分钟，覆盖 IPRULES vNext 的最小 datapath 闭环 + pkt stream 断言（实现：`tests/device-modules/ip/run.sh --profile smoke`）。用于开发期高频回归。
