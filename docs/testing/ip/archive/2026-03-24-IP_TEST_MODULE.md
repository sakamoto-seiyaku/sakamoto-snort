...
这部分不是“最终定稿”，而是把当前阶段已经确认的原则、候选 workload 和被放弃的方向先固定在文档里，避免后续上下文丢失。总原则见 `docs/testing/archive/TEST_COMPONENTS_MANIFESTO.md` 的 `4.4.3`；这里补充当前落地到 `tests/device-modules/ip/` 的具体形态。
