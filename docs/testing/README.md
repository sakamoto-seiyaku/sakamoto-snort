...
- `Host`：本机 `gtest/CTest`；`ASAN` / `UBSan` 是同一套 `Host` 用例的构建变体
- `Device / DX`：ADB + root 真机脚本、平台 smoke、测试模组、perf/longrun

说明：

- 仓库里现有 `p0/p1/p2` 只是历史 `CTest` label，不再作为当前路线图阶段名
- 当前默认开发 preset 已设置 `SNORT_ENABLE_DEVICE_TESTS=OFF`，因此日常 `dev-debug` / `host-asan-clang` / `host-ubsan-clang` 先聚焦 `Host`

## 当前优先阅读

- `docs/testing/HOST_TEST_SURVEY.md`
  - 当前 `Host` 端测试现状、sanitizer lane、覆盖缺口
...
如果只想直接在 UBSan build dir 内跑（跳过 wrapper）：
...
- `tests/device/ip/run.sh --profile perf` 当前在默认 `IPTEST_PERF_TRAFFIC_RULES=2000` 的大 ruleset 下可能出现 `BLOCKED: ... IPRULES.APPLY transport failed (rc=126)`；这属于 SNORT-17 Authoring Layer 落地前的 pre-SNORT-10 direct-apply current-head evidence，不是 SNORT-10 新 mutation contract。可临时用 `IPTEST_PERF_TRAFFIC_RULES=200` 验证链路；若仍影响架构判断，纳入后续 Authoring Layer / transport 切片。
