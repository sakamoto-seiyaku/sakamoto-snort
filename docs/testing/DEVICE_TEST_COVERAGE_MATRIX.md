...
> SNORT-10 一致性备注：本文保留部分 pre-SNORT-10 current-head / frozen 测试证据。后续拆实现 issue 时，packet-side 诊断目标是 `DIAGNOSTICS.START(channel=packet)` / Packet Diagnostics，不是 generic `STREAM.START(type=pkt|activity)`、持久 `tracked`、`wouldRuleId/wouldDrop` 或旧 traffic metrics。
...
| pre-SNORT-10 activity stream evidence（frozen/current-head） | — | ✅（legacy `STREAM.START type=activity` + started notice + STOP ack barrier；SNORT-10 不扩展该 surface） | — | ✅（legacy stream surface tests；SNORT-10 packet target 另见 Packet Diagnostics） |
| Packet Diagnostics / packet explain（SNORT-10 target） | — | — | —（当前 Tier-1 仍以 pre-SNORT-10 pkt stream 证据为主；SNORT-10 应改为 `DIAGNOSTICS.START(channel=packet)` + `diagnostic.packet.final.ruleMode`） | ◐（host 侧需补 diagnostics surface/state） |
| DNS stream 事件字段（frozen current-head） | — | ✅（netd inject → `DnsListener` → legacy DNS stream；`VNT-10b*` + `VNT-DOM-03/04/06/07/09`） | — | ◐（host 侧可测 frozen DNS stream state；不等于真机 DNS 链路） |
...
| config: `CONFIG.GET/SET`（device/app keys） | — | ✅（`block.enabled`、SNORT-10 `perfmetrics.level`；pre-SNORT-10 `tracked` 仅作 legacy evidence） | ✅（`block.enabled/iprules.enabled/block.ifaceKindMask`；pre-SNORT-10 `tracked` 不作为新目标） | ✅ |
...
| datapath correctness：verdict + per-rule stats + reasons | — | — | ✅（allow/block/observe-mode attribution/IFACE_BLOCK、`block.enabled=0`、`iprules.enabled=0`、payload bytes、conntrack allow/block；stats `hitPackets/hitBytes`） | ◐（host 侧不具备 NFQUEUE/iptables 真实链路） |
| metrics / observability surface（perf/reasons/Traffic Windows/conntrack） | — | ✅（shape + 部分 best-effort 触发/RESET；旧 `METRICS.GET(name=traffic)` 仅作 pre-SNORT-10 evidence） | ✅（IP cases 对 reasons 与旧 traffic bucket 做 current-head 断言；SNORT-10 应迁移到 Traffic Windows / reasons） | ✅ |
| Traffic Windows：受控触发 + RESET（per-app） | — | ◐（SNORT-10 target；旧 traffic metrics baseline 中有 best-effort 触发） | ◐（SNORT-10 target；旧 Tier-1 `traffic.txp/rxp/rxb` bucket 断言需迁移） | ◐（需补 `TRAFFIC_WINDOWS.*` shape + reset 语义） |
...
| `perfmetrics.level=off` 时“近似零成本”断言 | ✅（pre-SNORT-10 bool evidence；SNORT-10 target 为 level=off） | — | — | ◐（perf 场景会对比 off/basic/detail，但更偏相对趋势） | — |
| `perfmetrics.level=basic/detail` 时 latency samples 增长 | ✅（下载流量下 samples 增长；SNORT-10 public field 为 `packetVerdictLatencyUs`） | — | — | ✅（Tier-1 受控打流 + 记录 latency/rate） | ✅（长跑持续观测） |
| DNS perf（pre-SNORT-10 `dns_decision_us` evidence） | ◐（依赖 netd resolv hook；不活跃时会 SKIP DNS 断言） | — | — | — | — |
...
| --- | --- | --- |
| `perfmetrics.level=off` under Tier‑1 payload | ✅（pre-SNORT-10 `VNXOTH-01b~01d` 可作历史 evidence；SNORT-10 target 为 level=off） | — |
| `perfmetrics.level=basic/detail` samples 增长 + 幂等 | ✅（pre-SNORT-10 `VNXOTH-01e~01i` 可作迁移 evidence） | — |
| `perfmetrics.level` 非法值拒绝 | ✅（pre-SNORT-10 `VNXOTH-01j~01k` 可作迁移 evidence） | — |
| pre-SNORT-10 `dns_decision_us` perf | ◐（`VNXOTH-01l`；仅 optional/non-gate，不强依赖 netd hook） | — |
| `DOMAINLISTS.IMPORT` under/over limits + `HELLO` recovery | ✅（`VNXOTH-02a~02f`；optional limits sanity） | — |
| pre-SNORT-10 direct `IPRULES.APPLY` recommended/hard limits + all-or-nothing + recovery | ✅（`VNXOTH-02g~02n`；optional limits sanity / current-head evidence） | — |
...
- pre-SNORT-10 direct IP perf 大 ruleset apply：默认 `IPTEST_PERF_TRAFFIC_RULES=2000` 可能触发 `IPRULES.APPLY transport failed`；这是 current-head evidence，不是 SNORT-10 新 mutation contract，可临时降到 `IPTEST_PERF_TRAFFIC_RULES=200` 验证链路（见 `docs/testing/README.md` 与 `docs/testing/PERFORMANCE_TEST_RECORD.md`）。

---

## 4) 覆盖分类（可测但未测项进入 Plane）

本节不作为本地 backlog。若某项需要推进，先在 Plane `SNORT` 建立 work item，再回写本文作为覆盖状态或证据索引。
...
### Gap-02（已补齐，pre-SNORT-10 current-head）：DNS stream 端到端事件（不依赖公网）
- 现状（已解决）：`dx-smoke-control` 通过 **netd inject** 直接触发 `DnsListener`，并在 frozen DNS stream 下断言收到匹配事件、字段、suppressed notice、旧 `traffic.dns` 与 `domainSources` bucket。SNORT-10 packet-side 重构不扩展 DNS stream；Domain/DNS line 后续单独整理。
- 入口：
...
### Gap-03（已补齐，pre-SNORT-10 current-head）：旧 `METRICS.GET(name="traffic")` 的稳定可控触发
- 现状（已解决）：`dx-smoke-datapath`（IP Tier‑1）已在受控打流下断言：
  - 旧 `METRICS.RESET(name=traffic,app)` 能清零
...
- 现状：`dx-smoke-control` 的 `VNT-DOM-08` 在 hook 就绪时触发真实 DNS lookup 并断言 frozen DNS stream、旧 `traffic.dns.block` 与 `domainSources`；hook 不活跃时明确 `BLOCKED` 并提示 `dev/dev-netd-resolv.sh status|prepare`。
...
- 现状：Host 已覆盖 `DOMAINLISTS.IMPORT` limits（`maxImportBytes/maxImportDomains`）与 pre-SNORT-10 direct `IPRULES.APPLY` preflight hard-limit 的结构化错误；Device 侧通过 `dx-casebook-other` 的 `VNXOTH-02*` 做 optional sanity。
