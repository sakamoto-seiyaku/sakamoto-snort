...
- `tests/device/ip/run.sh --profile perf` 在默认 `IPTEST_PERF_TRAFFIC_RULES=2000` 下的 large ruleset apply failure（`rc=126`）如仍重要，应先在 `SNORT-10` 中判断它属于架构边界、transport 限制，还是普通 bug。
