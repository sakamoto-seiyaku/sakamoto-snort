...
- `l4Status`：string（仅 `type="pkt"` 事件；always-present），取值锁死为：`known-l4|other-terminal|fragment|invalid-or-unavailable-l4`。用于解释端口与 L4 可用性；前端不得仅凭 `protocol="other"` 推断“合法 other terminal”，必须结合 `l4Status="other-terminal"` 判断。
- `srcPort/dstPort`：always-present number；端口不可用时输出 `0`，并必须通过 `l4Status` 解释“端口不可用”（不得把 `0` 当作真实端口）。
...
- `type="pkt"`（packet 判决事件）最小字段：
  ...
  - 时间：`timestamp`
  - 归属：`app/uid/userId`
  - 输入（五元组与上下文）：`scope?/direction/ipVersion/protocol/l4Status/srcIp/dstIp/srcPort/dstPort/length/ifindex/ifaceKindBit/interface?`
...
- v1 顶层字段固定为：
  ...
  - `overflowDrops`
- v1 新增 `byFamily`：
  - `byFamily.ipv4` 与 `byFamily.ipv6` 两项都必须存在
  - 每个 family 分项完整包含上述四字段（uint64）
...
{"id":1,"ok":true,"result":{"conntrack":{
  ...
  "overflowDrops":0,
  "byFamily":{
    "ipv4":{"totalEntries":0,"creates":0,"expiredRetires":0,"overflowDrops":0},
    "ipv6":{"totalEntries":0,"creates":0,"expiredRetires":0,"overflowDrops":0}
  }
}}}
```
