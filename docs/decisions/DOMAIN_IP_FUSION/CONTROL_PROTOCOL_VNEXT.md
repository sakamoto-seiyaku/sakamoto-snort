...
- server 必须设置硬上限：`maxFrameBytes`（实现常量；用于防 OOM/DoS）。
- `HELLO` 必须回显 `maxFrameBytes`（便于前端/脚本在本地提前拦截并提示用户）。
- 若 `len > maxFrameBytes`（已确认；2.37‑A）：立即断开连接（不返回 response frame；可记录 server log）。
- 备注（易用性）：由于 `id` 在 payload 内，`len>maxFrameBytes` 时无法返回带 `id` 的结构化 response；  
  但实现可选在断连前输出一条不带 `id/ok` 的 `type="notice"` 事件（例如 `notice="frameTooLarge"`，回显 `len/maxFrameBytes`），用于提升排障体验（不改变安全边界）。
...
- stream connection 上（命令入口统一为 `STREAM.START/STREAM.STOP`）：
  ...
  - `STREAM.STOP` 的 response frame 是 ack barrier：ack 之后不得再输出任何事件/notice，直到下一次 `STREAM.START`（或连接关闭）。
