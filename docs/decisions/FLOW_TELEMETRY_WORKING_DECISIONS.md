...
- `flowInstanceId` 生成采用“**分配式**”：在 entry 创建时一次性生成并写入 entry（例如基于 daemon/conntrack 的 instance/epoch + 局部序号）。records 中单独携带 timestamp 字段用于排序/时间线；不要求 `flowInstanceId` 可排序。
...
- 资源压力下的降级原则：
  ...
  - 若仍然满，则拒绝创建新的 telemetry/flow entry，并增加 `resourcePressure` counter；已有 entry 的更新照常进行。
...
- END reason 口径：
  ...
  - active timeout / max export interval 触发的是 UPDATE，不是 END。
  - RESETALL、`TELEMETRY.CLOSE`、session rebuild 已定义为 telemetry session boundary，前端先停止 ingest 并清库/截断活跃状态；不要求为每个 active flow 逐条发送 `TELEMETRY_DISABLED` END。
