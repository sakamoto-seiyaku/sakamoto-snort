...
- **P2 真机集成 / smoke / 兼容性验证 / perf**：见 `tests/integration/device-smoke.sh`、`tests/integration/perf-network-load.sh`、`tests/integration/iprules-device-matrix.sh`（runbook：`docs/testing/IPRULES_DEVICE_VERIFICATION.md`）、`tests/integration/full-smoke.sh`
...
- ⏳ IPRULES v1（关闭 change 的必要补偿项）：真机规则矩阵验证与记录（`docs/testing/IPRULES_DEVICE_VERIFICATION.md` + `tests/integration/iprules-device-matrix.sh`）
